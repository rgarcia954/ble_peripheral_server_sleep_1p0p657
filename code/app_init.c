...
void App_GPIO_Config(void)
{
    ...
    /* Configure Wakeup activity GPIO for monitoring RTC wakeup */
    SYS_GPIO_CONFIG(WAKEUP_ACTIVITY_RTC, (GPIO_MODE_GPIO_OUT | GPIO_LPF_DISABLE |
                                          GPIO_WEAK_PULL_UP  | GPIO_6X_DRIVE));
}
...
void Print_Time_Info(uint64_t total_rtc_cycles)
{
	...
	/* Initialize swmtrace when wakeup from of sleep */
	swmTrace_init(traceOptions, 8);
	swmLogInfo("\033[2J\033[H");
	swmLogInfo("\n\rRTC Cycles: %d\n\r", total_rtc_cycles);
	swmLogInfo("\n\rDays: %d\n\rHours: %d\n\rMinutes: %d\n\rSeconds: %d\n\rMilliseconds: %d\n\r",
